...
def normalize_extracted_fields(report: dict[str, str]) -> None:
    ...
    # Operational briefings should not include community meeting text.
    op_brief = report.get("Operational_Briefings", "")
    if op_brief:
        for marker in ["Community Meeting", "Livestream:"]:
            marker_pos = op_brief.find(marker)
            if marker_pos != -1:
                op_brief = op_brief[:marker_pos].strip()
        op_brief = re.sub(r"Operational Briefings\s*-\s*", "Operational Briefings - ", op_brief)
        op_brief = re.sub(r"\s*-\s*\n\s*", " - ", op_brief)
        report["Operational_Briefings"] = clean_field_text(op_brief)

    # Livestream should keep prefix as in examples.
    livestream = report.get("Community_Meeting_Livestream", "")
    if livestream and not livestream.lower().startswith("livestream:"):
        report["Community_Meeting_Livestream"] = f"Livestream: {livestream}"

    # Curfew order should start with the sheriff label and stop before zone links.
    curfew = report.get("Curfew_Order", "")
    if curfew:
        prefix = "Per the Los Angeles County Sheriff's Department:"
        idx = curfew.lower().find(prefix.lower().rstrip(":"))
        if idx != -1:
            curfew = curfew[idx:]
        for marker in ["To identify your evacuation zone", "To receive updates for your evacuation zones"]:
            pos = curfew.find(marker)
            if pos != -1:
                curfew = curfew[:pos].strip()
        if curfew and not curfew.startswith(prefix):
            curfew = f"{prefix} {curfew}".strip()
        report["Curfew_Order"] = clean_field_text(curfew)

    # Evacuation warning zones should keep only zone list content when possible.
    warning_zones = report.get("Evacuation_Warning_Zones", "")
    if warning_zones:
        warning_zones = warning_zones.replace("The Following Zones are under Evacuation Warning:", "").strip()
        open_res_pos = warning_zones.find("OPEN TO RESIDENTS ONLY!")
        if open_res_pos != -1:
            warning_zones = warning_zones[:open_res_pos].strip()
        report["Evacuation_Warning_Zones"] = clean_field_text(warning_zones)

    warning_open = report.get("Evacuation_Warning_Open_To_Residents", "")
    if warning_open:
        marker = "OPEN TO RESIDENTS ONLY!"
        pos = warning_open.find(marker)
        if pos != -1:
            warning_open = warning_open[pos + len(marker) :].strip()
        report["Evacuation_Warning_Open_To_Residents"] = clean_field_text(warning_open)

    # Keep evacuation orders details focused on details (resident access etc).
    orders_details = report.get("Evacuation_Orders_Details", "")
    if orders_details:
        resident_pos = orders_details.find("Resident Access:")
        if resident_pos != -1:
            orders_details = orders_details[resident_pos:].strip()
        else:
            candidates: list[int] = []
            for start_marker in [
                ...
                pos = orders_details.find(start_marker)
                if pos != -1:
                    candidates.append(pos)
            if candidates:
                orders_details = orders_details[min(candidates):].strip()
        report["Evacuation_Orders_Details"] = clean_field_text(orders_details, keep_blank_lines=True)

    # Trim these fields before any of their downstream markers.
    trim_before_markers = {
        "Situation_Summary": [
    ...
    road = report.get("Road_Closure_Sources", "")
    if road:
        road = road.replace("Follow the links below to stay up-to-date on road closures for the Palisades Fire.", "").strip()
        report["Road_Closure_Sources"] = clean_field_text(road)

    # Cooperating agencies should not include table headers.
    coop = report.get("Cooperating_Agencies", "")
    if coop:
        for token in ["Name", "Phone", "Address", "URL"]:
            coop = re.sub(rf"\b{token}\b", "", coop)
        report["Cooperating_Agencies"] = clean_field_text(coop)
...
def fetch_pdf_page_count(pdf_url: str) -> str:
    try:
...
def extract_pdf_metadata(content: Tag) -> tuple[str, str]:
    for anchor in content.find_all("a", href=True):
        href = anchor["href"]
        if ".pdf" not in href.lower():
            continue
        pdf_url = urljoin(BASE_URL, href)
        file_name = unquote(Path(urlparse(pdf_url).path).name)
        return file_name, fetch_pdf_page_count(pdf_url)
    return "", ""
...
def parse_report(
    ...
    file_name, page_count = extract_pdf_metadata(content)
